...
class WebGLRenderer {
  ...
  init() {
    ...
    (prog as Record<string, unknown>).resolution = gl.getUniformLocation(prog, 'resolution');
    (prog as Record<string, unknown>).time = gl.getUniformLocation(prog, 'time');
    (prog as Record<string, unknown>).move = gl.getUniformLocation(prog, 'move');
    (prog as Record<string, unknown>).touch = gl.getUniformLocation(prog, 'touch');
    (prog as Record<string, unknown>).pointerCount = gl.getUniformLocation(prog, 'pointerCount');
    (prog as Record<string, unknown>).pointers = gl.getUniformLocation(prog, 'pointers');
  }
  ...
  render(now = 0) {
    ...
    const p = prog as Record<string, WebGLUniformLocation>;
...
class PointerHandler {
  ...
  private _moves: [number, number] = [0, 0];
  ...
  get move(): [number, number] { return this._moves; }
  get coords(): number[] {
...
function useShaderBackground() {
  ...
  useEffect(() => {
    ...
    if (rendererRef.current.test(DEFAULT_SHADER) === null) {
      // shader compiled successfully, already loaded
    }
